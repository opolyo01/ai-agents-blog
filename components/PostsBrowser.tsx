...
type ViewMode = "topic" | "date";

export default function PostsBrowser({ posts }: PostsBrowserProps) {
  ...
  return (
    <section className="mt-12">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <p className="text-sm font-medium uppercase tracking-[0.24em] text-gray-500 dark:text-gray-400">
            {view === "topic" ? "Browse by Topic" : "Browse by Date"}
          </p>
          <h2 className="mt-2 max-w-3xl text-2xl font-semibold tracking-tight sm:text-3xl">
            {view === "topic" ? "Pick a track and explore inside it" : "Everything, newest first"}
          </h2>
        </div>
        ...
        <div
          ...
          className="inline-flex shrink-0 items-center gap-1 rounded-full border border-gray-200 bg-white p-1 dark:border-white/10 dark:bg-white/5"
        >
          <ViewToggleButton active={view === "topic"} onClick={() => setView("topic")}>
            By Topic
          </ViewToggleButton>
          <ViewToggleButton active={view === "date"} onClick={() => setView("date")}>
            By Date
          </ViewToggleButton>
        </div>
      </div>
      ...
      {view === "topic" ? (
        <TopicView categoryEntries={categoryEntries} />
      ) : (
        <DateView posts={posts} />
      )}
    </section>
  );
}
...
function TopicView({ categoryEntries }: { categoryEntries: [string, PostMeta[]][] }) {
  return (
    <>
                  ...
                  >
                    ...
                    <h4 className="mt-2 text-lg font-semibold tracking-tight leading-tight">
                      ...
                      >
                        ...
                        <span aria-hidden="true" className={`text-base transition-colors ${meta.arrowClass}`}>
                          ↗
                        </span>
                      </Link>
                    </h4>
...
function DateView({ posts }: { posts: PostMeta[] }) {
  return (
    <div className="mt-6 space-y-3">
      {posts.map((post) => {
        ...
        return (
          <article
            key={`${post.category}-${post.slug}`}
            className="rounded-2xl border border-gray-200 bg-white p-5 transition-transform hover:-translate-y-0.5 dark:border-white/10 dark:bg-white/5"
          >
            <div className="flex flex-wrap items-center gap-3">
              <p className="text-sm text-gray-500 dark:text-gray-400">{post.date}</p>
              <span className={`rounded-full px-2.5 py-0.5 text-xs font-medium ${meta.countClass}`}>
                {post.category}
              </span>
            </div>
            <h4 className="mt-2 text-lg font-semibold tracking-tight leading-tight sm:text-xl">
              <Link
                href={`/posts/${post.slug}`}
                className="group inline-flex items-center gap-2 transition-colors hover:text-gray-600 dark:hover:text-gray-300"
              >
                <span>{post.title}</span>
                <span
                  aria-hidden="true"
                  className="text-base text-gray-300 transition-colors group-hover:text-gray-600 dark:text-gray-600 dark:group-hover:text-gray-300"
                >
                  ↗
                </span>
              </Link>
            </h4>
            <p className="mt-2 text-base leading-7 text-gray-700 dark:text-gray-200">{post.summary}</p>
          </article>
        );
      })}
    </div>
